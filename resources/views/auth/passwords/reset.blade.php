...
<head>
    ...
    <!-- End Meta --> 
    <!-- Title -->
    <title>Reset | Library - ICT Cortex student project</title>
    ...
    <main class="h-screen small:hidden bg-login">
        <div class="flex items-center justify-center pt-[13%]">
            <div class="w-full max-w-md">
                <form method="POST" action="{{ route('password.update') }}" class="px-12 pt-6 pb-4 mb-4 bg-white rounded shadow-lg" >
                    @csrf
                    ...
                    @endif
                    <div class="mb-6">
                        <label for="email" class="block mb-2 text-sm font-normal text-gray-700">{{ __('E-Mail Address') }}</label>
                        <input id="email" type="email"
                               class="w-full px-3 py-2 mb-3 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline @error('email') is-invalid @enderror"
                    ...
                    <div class="mb-6">
                        ...
                        <input
                            class="w-full px-3 py-2 mb-3 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline @error('password') is-invalid @enderror"
                            name="password_confirmation" required autocomplete="current-password" placeholder="Password" 
                            id="password" type="password"/>
